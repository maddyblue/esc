...
type Config struct {
	...
	Private bool
	//If true, write each input file into a seperate file
	Split bool
...
var fnameSplitter = regexp.MustCompile(`(.+)\.go`)
...
func Run(conf *Config) {
	...
	if conf.Split && conf.OutputFile == "" {
		log.Fatal("split mode requires -o")
	}
	...
	for i, fname := range fnames {
		...
		compressed := segment(&buf)
		if conf.Split {
			m := fnameSplitter.FindStringSubmatch(conf.OutputFile)
			if m == nil || len(m) < 2 {
				log.Fatal("could not determine file names for split mode")
			}
			dfname := fmt.Sprintf("%s-%d.go", m[1], i)
			dfWriter, err := os.Create(dfname)
			if err != nil {
				log.Fatal(err)
			}
			dataParams := struct{ PackageName, Counter string }{
				PackageName: conf.Package,
				Counter:     fmt.Sprintf("%d", i),
			}
			tmpl, err := template.New("").Parse(splitFile)
			if nil != err {
				log.Fatal(err)
			}
			err = tmpl.Execute(dfWriter, dataParams)
			if nil != err {
				log.Fatal(err)
			}
			fmt.Fprint(dfWriter, compressed)
			compressed = fmt.Sprintf("_escData%d", i)
			err = dfWriter.Close()
			if nil != err {
				log.Fatal(err)
			}
	...
	%q: {
		...
		modtime: %v,
		compressed: %s,
	},%s`, fname, f.local, len(f.data), t, compressed, "\n")
	}
	for d := range dirs {
...
const (
	headerTemplate = `// This file is automatically generated using github.com/mjibson/esc.
package {{.PackageName}}
...
`
	...
	splitFile = `// This file is automatically generated using github.com/mjibson/esc.

package {{.PackageName}}

var _escData{{.Counter}} =`
)
